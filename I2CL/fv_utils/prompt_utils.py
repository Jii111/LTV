...
# 추가
def convert_basetask_to_icldataset(task_obj, sample_indexes = None):
    """
    Convert any BaseTask (AGNews, SST2, Subj, MR, TREC, etc.)
    into Function Vector-style ICLDataset using the task's apply_template().

    - input : task_obj.apply_template(data)[0]
    - output: label-indexed answer from answer_list
    """
    inputs, outputs = [], []
    
    if sample_indexes is not None:
        data_iter = [task_obj.all_data[i] for i in sample_indexes]
        print(f"++ train(run demo({len(data_iter)}={len(sample_indexes)}) : ", data_iter)
    else:
        data_iter = task_obj.all_data

    for data in data_iter:
        try:
            input_str, answer_list, label = task_obj.apply_template(data)

            if isinstance(label, int) and label < len(answer_list):
                output_str = answer_list[label]
            else:
                output_str = str(label)
        except Exception as e:
            print(f"[WARN] apply_template() failed for sample {data}: {e}")
            continue
    ...
    return ICLDataset(df)


def load_dataset_from_i2cl(task_name: str, index_dict : dict, run_time : int,
                           config: dict = None) -> Dict[str, ICLDataset]:
    """
    Load dataset from I2CL BaseTask classes and convert to FV-compatible ICLDataset format.
    Supports I2CL-style config arguments: max_data_num, sample_mode, seed, etc.
    """
    # ✅ config 기본값 세팅
    cfg = config or {}
    seed = cfg.get('seed', 42)
    sample_mode = cfg.get('sample_method', 'random')
    train_data_num = cfg.get('train_data_num', None)
    val_data_num = cfg.get('val_data_num', None)
    test_data_num = cfg.get('test_data_num', None)
    run_name = "run_" + str(run_time)
    
    sample_indexes = index_dict[run_name]

    dataset_dict = {}
    for split in ['train', 'validation', 'test']:
        max_data_num = (
            train_data_num if split == 'train'
            else val_data_num if split == 'validation'
            else test_data_num
        )

        task = md.get_dataset(task_name,
                           split=split,
        ...
        if split == 'train':
            dataset_dict[split] = convert_basetask_to_icldataset(task, sample_indexes)
        else:
            dataset_dict[split] = convert_basetask_to_icldataset(task)
    print(f"[Done] Loaded {task_name} from I2CL with config-based sampling and converted to FV format ✅")
    return dataset_dict
