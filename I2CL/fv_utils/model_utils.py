    ...
    if model_name == 'gpt2-xl':
        ...
        if '70b' in model_name.lower():
            ...
            model = AutoModelForCausalLM.from_pretrained(model_name,
                        load_in_8bit=True, torch_dtype=model_dtype, device_map="auto")
        ...
        MODEL_CONFIG = {
            "n_heads": model.config.num_attention_heads,           # 28
            "n_layers": model.config.num_hidden_layers,            # 28
            "resid_dim": model.config.hidden_size,                 # 3584
            "name_or_path": model.config._name_or_path,
            "attn_hook_names": [
            ...
            "prepend_bos": True  # Qwen은 BOS 토큰을 자동 추가하지 않으므로 True로 두는 게 안전
        }
    else:
