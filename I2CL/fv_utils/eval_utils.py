...
def compute_dataset_baseline(dataset, task, model, model_config, tokenizer, n_shots=10, seed=42, generate_str=False, metric=None, prefixes=None, separators=None) -> dict:
    """
    ...
    for N in range(n_shots+1):
        set_seed(seed)
        results_dict[N] = n_shot_eval_no_intervention(dataset, task_name = task, n_shots=N, model=model, model_config=model_config, tokenizer=tokenizer,
...
def n_shot_eval(dataset, task_name, fv_vector, edit_layer: int, n_shots: int, model, model_config, tokenizer, shuffle_labels:bool=False,
    ...
    for j in tqdm(range(len(dataset[test_type])), total=len(dataset[test_type]), disable=True):
        ...
        if n_shots == 0:
            ...
            word_pairs = dataset['train'][np.random.choice(len(dataset['train']),min(len(dataset['train']),n_shots), replace=False)]
        word_pairs_test = dataset[test_type][j]
...
def n_shot_eval_no_intervention(dataset, task_name, n_shots, model, model_config, tokenizer, compute_ppl=True, generate_str=False,
    ...
    for j in tqdm(range(len(dataset[test_split])), total=len(dataset[test_split]), disable=True):
        if n_shots == 0:
            word_pairs = {'input':[], 'output':[]}
        else:
        ...
        pred_label, true_label, logit = eval_i2clst(task_name, sentence, ans_txt_list = ans_txt_list, interest_index = interest_index, target = [target], model=model, tokenizer=tokenizer)
        all_preds.append(pred_label); all_labels.append(true_label)
        all_pred_logits.append(logit)
        all_inputs.append(sentence)
...
# 수정
def eval_i2clst(task_name, sentence, ans_txt_list,interest_index, target, model, tokenizer, fv_result = None):
    device = model.device
    inputs = tokenizer(sentence, return_tensors='pt').to(device)
    original_pred_idx = len(inputs.input_ids.squeeze()) - 1
    ...
    pred_logits = output[:, interest_index]  # (1, num_classes)
    probs = F.softmax(pred_logits, dim=-1)
    pred_label = probs.argmax(dim=-1).item()
    
    true_label = None
    for i, label_text in enumerate(ans_txt_list):
        if label_text.strip() in target[0].strip():
            true_label = i
            break

    return pred_label, true_label, pred_logits.detach().cpu()
...
def compute_diff_entropy(logits_p, logits_q, is_qwen=False):
    probs_p= F.softmax(logits_p, dim=-1)
    probs_p = probs_p.clamp_min(1e-12)
    entropy_values_p = -(probs_p * probs_p.log()).sum(dim=-1)
    
    probs_q= F.softmax(logits_q, dim=-1)
    probs_q = probs_q.clamp_min(1e-12)
    entropy_values_q = -(probs_q * probs_q.log()).sum(dim=-1)
    
    diff_entropy_values = entropy_values_p - entropy_values_q
    
    if is_qwen == True:
        return diff_entropy_values.float().mean().item(), diff_entropy_values.float().cpu().numpy()
    else:
        return diff_entropy_values.mean().item(), diff_entropy_values.cpu().numpy()