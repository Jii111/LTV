        ...
        try:
            ...
            for layer_idx, layer in enumerate(self.inject_layers):
                for module_idx, module in enumerate(config['module']):
                    ...
                    inject_func = self.inject_hook_func(
                        context_vector_container, strength,
                ...
                if inject_pos == 'all':
                    ...
                    if inject_pos == 'last':
                        for i in range(output.size(0)):
                            ...
                            content = strength[1] * output[i, end_idx, :].clone().detach() + strength[
                                0] * context_vector[i, end_idx, :]
                        ...
                        for i in range(output.size(0)):
                            ...
                            content = strength[1] * output[i, random_idx, :].clone().detach() + strength[
                                0] * context_vector[i, random_idx, :]
    ...
    def calibrate_strength(self, context_vector_dict, dataset, config,
        ...
        if config['optim'] == 'sgd':
            optimizer = torch.optim.SGD(
                optim_paramters, lr=config['lr'],
                weight_decay=config['wd']
                )
        elif config['optim'] == 'adamW':
            optimizer = torch.optim.AdamW(
                optim_paramters, config['lr'],
                weight_decay=config['wd']
                )
        ...
        with self.inject_latent(
                context_vector_dict, config,
            ...
            for _ in range(epochs):
                ...
                for i in range(epoch_iter):
                    ...
                    print(
                        f'Epoch {_ + 1}/{epochs}, batch {i // batch_size + 1}/{len(all_data) // batch_size + 1}, loss: {loss.item()}, lr: {cur_lr}'
                        )
        ...
        for _ in range(epochs):
            ...
            for i in range(0, len(all_data), batch_size):
                ...
                if 'qwen' in self.tokenizer.__class__.__name__.lower():
                    pred_logits = logits[torch.arange(logits.size(0)), pred_loc].float() # 수정 qwen
                else:
                    pred_logits = logits[torch.arange(logits.size(0)), pred_loc]
                # get loss
                gt_label = torch.tensor([label_map[label] for label in batch_label]).to(self.device)
...
class Qwen3Wrapper(ModelWrapper):
    def __init__(self, model, tokenizer, model_config, device):
        ...
        #self.embed_matrix = self.model.model.embed_tokens.weight.data
        self.embed_matrix = None if hasattr(model, "get_base_model") else base.model.embed_tokens.weight.data
