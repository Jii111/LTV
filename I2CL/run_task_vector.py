...
def main(args):
    ...
    model_wrapper = utils.get_model_wrapper(args.model_name, model, 
                                            ...
                                            args.device)
    # load datasets
    train_dataset = md.get_dataset(args.dataset_name, split='train', max_data_num=None, seed=args.config['seed']) # ✅ 수정
    val_dataset = md.get_dataset(args.dataset_name, split='validation', 
                                 max_data_num=args.config['val_data_num'],
                                 sample_mode=args.config['sample_method'], seed=args.config['seed'])
    ...
    # quick patch: regard shot_per_class as if 
    ...
    for run_id in range(args.config['run_num']):
        ...
        demon_data_index = index_dict[run_name]
        ...
        if args.config['run_baseline']:
            ...
            test_fewshot_result, test_fewshot_logits, test_fewshot_labels = test_evaluator.evaluate(model_wrapper, tokenizer, 
                                                          demonstration=baseline_demon, 
                                                          use_cache=args.config['use_cache'], return_logits=True, logits_mode=args.config['logits_mode']) # ✅ 수정
        ...
        if 'Qwen' in args.model_name:
            mean_kl, kl_values = utils.compute_kl_divergence(test_fewshot_logits, test_ours_logits, is_qwen=True)
        else:
            mean_kl, kl_values = utils.compute_kl_divergence(test_fewshot_logits, test_ours_logits)
    ...
    del val_evaluator, test_evaluator, result_dict, context_vector_dict, kl_dict
