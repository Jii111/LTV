...
def collect_zero_and_delta(
    ...
    Returns (features, targets) both on CPU.
    """
    feature_list = []
    target_list = []
...
def main(args):
    ...
    for run_id in run_progress:
        ...
        zero_hidden = zero_hidden.to(args.device)
        ...
        m2a_hidden = m2a_hidden.to(args.device)
        ...
        if mlp_cfg.get('enabled', True):
            ...
            with torch.no_grad():
                ...
                for i in range(0, zero_hidden.size(0), bs):
                    batch = zero_hidden[i:i + bs]
                    out = mlp_model(inputs=batch)
                    mlp_delta.append(out['logits'])
                mlp_delta = torch.cat(mlp_delta, dim=0)
                m2mlp_hidden = zero_hidden + mlp_delta
        else:
            mlp_logs = []
