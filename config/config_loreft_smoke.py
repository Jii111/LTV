...
Exercises the shipped code path (lmse objective, all-layer rank-8
intervention, ICL-target collection, injection, metrics, curve/info JSON)
in ~10 min on one GPU. Expected sanity marks: EXIT=0; result_dict.json has
test_result['loreft'][0]['lmse'] with 'curve' entries whose ortho_err ~ 1e-7
and rotate_drift/source_drift > 0; train_final_lr follows warmup->decay;
acc within a few points of zero-shot is fine at this budget — the smoke
